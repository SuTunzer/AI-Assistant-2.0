...
import { modelsFor } from '../../../packages/domain/src/budget.js';
...
export async function saveSettings(value: unknown) {
  ...
  // forget. Extraction and transcription run on Gemini whatever advice uses.
  const gemini = modelsFor('gemini');
  if (
    !modelsFor(parsed.adviceProvider).includes(parsed.adviceModel) ||
    !gemini.includes(parsed.extractionModel) ||
    !gemini.includes(parsed.transcriptionModel)
  )
  ...
  for (let i = 0; i < memories.length; i++)
    for (let j = i + 1; j < memories.length; j++) {
      ...
      if (score >= (basis === 'meaning' ? 0.86 : 0.6))
        pairs.push({ ids: [a.id, b.id], score: Math.round(score * 100) / 100, basis });
    }
  return pairs.sort((x, y) => y.score - x.score).slice(0, 25);
}
